...
public:
	...
	Paginator(Iterator first, Iterator last, size_t page_size) {
		while (first != last) {
			if (last - first > page_size) {
				Iterator nxt = first + page_size;
				m_pages.push_back(Page(first, nxt));
				first = nxt;
			}
			else {
				m_pages.push_back(Page(first, last));
				first = last;
			}
		}
	}
	...
	auto begin() const { return m_pages.begin(); }
	auto end() const { return m_pages.end(); }
