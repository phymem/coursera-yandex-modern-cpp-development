...
public:
	...
	AirportCounter(TIterator begin, TIterator end)
	{
		InitItems();
		while (begin != end) {
			m_items[A2U(*begin++)].second++;
		}
	}

	// получить количество элементов, равных данному
	size_t Get(TAirport airport) const {
		return m_items[A2U(airport)].second;
	}

	// добавить данный элемент
	void Insert(TAirport airport) {
		m_items[A2U(airport)].second++;
	}
	...
	void EraseAll(TAirport airport) {
		m_items[A2U(airport)].second = 0;
	}

	// convert TAirport to UInt
	constexpr static unsigned int A2U(TAirport airport) { return static_cast<unsigned int>(airport); }
	constexpr static TAirport U2A(unsigned int airport) { return static_cast<TAirport>(airport); }

	using Item = pair<TAirport, size_t>;
	using Items = array<Item, A2U(TAirport::Last_)>;
