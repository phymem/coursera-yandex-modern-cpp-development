...
void TestCoursera_1() {
	istringstream is("You hardcode tests, right?\n");
	Lexer lex(is);

	ASSERT_EQUAL(lex.CurrentToken(), Token(TokenType::Id{"You"}));
	ASSERT_EQUAL(lex.NextToken(), Token(TokenType::Id{"hardcode"}));
	ASSERT_EQUAL(lex.NextToken(), Token(TokenType::Id{"tests"}));
	ASSERT_EQUAL(lex.NextToken(), Token(TokenType::Char{','}));
	ASSERT_EQUAL(lex.NextToken(), Token(TokenType::Id{"right"}));
	ASSERT_EQUAL(lex.NextToken(), Token(TokenType::Char{'?'}));
...
void RunLexerTests(TestRunner& tr) {
	...
	RUN_TEST(tr, Parse::TestCoursera_1);
}
