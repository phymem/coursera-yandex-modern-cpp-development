...
const std::streamsize POSITION_PRECISION = 16;
...
public:
	...
	friend std::ostream& operator << (std::ostream& os, const Circle& circle) {
		...
		return os << std::setprecision(POSITION_PRECISION)
			<< "<circle cx=\\\"" << circle.center.x
			<< "\\\" cy=\\\"" << circle.center.y
...
public:
	...
	friend std::ostream& operator << (std::ostream& os, const Text& text) {
		std::streamsize default_precision = os.precision();
		os << std::setprecision(POSITION_PRECISION)
			<< "<text x=\\\"" << text.point.x
			<< "\\\" y=\\\"" << text.point.y
			<< std::setprecision(default_precision)
			<< "\\\" dx=\\\"" << text.offset.x
...
public:
	...
	friend std::ostream& operator << (std::ostream& os, const Polyline& line) {
		std::streamsize default_precision = os.precision();
		os << "<polyline points=\\\"" << std::setprecision(POSITION_PRECISION);
		...
		return os << std::setprecision(default_precision)
			<< "\\\" " << static_cast<Polyline::base_cref_t>(line) << " />";
	}
