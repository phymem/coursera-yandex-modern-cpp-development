	...
	for (const Json::Node& node : base_requests) {
		...
		if (dict.at("type").AsString() == "Bus") {
			...
			for (size_t i = 1; i < bus->stops.size(); ++i) {
				...
				Stop* s2 = bus->stops[i];
				s1->is_neighbour.insert(s2->stop_id);
				s2->is_neighbour.insert(s1->stop_id);
			}
...
void Descriptions::PrepRender(
	...
	CalcGraphIdx([](const Stop* lhs, const Stop* rhs)
		{ return lhs->graph_pos.longitude < rhs->graph_pos.longitude; });
		double x_step = (width - 2 * padding) / stops_.back()->graph_idx;
	for (Stop* stop : stops_) {
	...
	double y_step = (height - 2 * padding) / stops_.back()->graph_idx;
	for (Stop* stop : stops_) {
