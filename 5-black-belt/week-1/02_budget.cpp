// g++ -std=c++1z 02_budget.cpp -v -fsanitize=address -g -fno-omit-frame-pointer -fno-optimize-sibling-calls
...
private:
	...
	struct Node {
		...
		Node(Node* left__, Node* right__, const IndexSegment& segment__)
			: left(left__), right(right__), segment(segment__) {}
		~Node() {
			delete left;
			delete right;
		}
	...
	static Node* Build(IndexSegment segment) {
		if (segment.empty()) {
			...
			return new Node(nullptr, nullptr, segment);
		} else {
