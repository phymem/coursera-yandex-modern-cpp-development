// g++ -std=c++1z 01_domains.cpp -v -fsanitize=address -g -fno-omit-frame-pointer -fno-optimize-sibling-calls
...
bool IsSubdomain(const Domain& subdomain, const Domain& domain) {
	...
	return subdomain.GetPartCount() >= domain.GetPartCount()
		// XXX && equal(begin(domain_reversed_parts), end(domain_reversed_parts), end(subdomain_reversed_parts));
...
public:
	...
	DomainChecker(InputIt domains_begin, InputIt domains_end) {
		// XXX sorted_domains_.resize(distance(domains_begin, domains_end));
	...
	bool IsSubdomain(const Domain& candidate) const {
		...
		// XXX return ::IsSubdomain(candidate, **it);
