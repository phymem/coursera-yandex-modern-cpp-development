...
public:
	...
	friend ostream& operator << (ostream& os, const HttpResponse& resp) {
		...
		if (resp.content.size()) {
			os << "Content-Length: " << to_string(resp.content.length()) << "\n";
		}
...
public:
	HttpResponse ServeRequest(const HttpRequest& req) {
		if (req.method == "POST") {
			if (req.path == "/add_user") {
				...
				return ServeAddCapture(req);
			}
		} else if (req.method == "GET") {
			if (req.path == "/user_comments") {
				return ServeUserComments(req);
			} else if (req.path == "/captcha") {
				return ServeCapture(req);
			}
		}
	...
	HttpResponse ServeAddCapture(const HttpRequest& req) {
	...
	HttpResponse ServeCapture(const HttpRequest& req) {
