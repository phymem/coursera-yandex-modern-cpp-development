...
private:
	...
	struct AssignmentReturn {
		std::vector<T>& coeffs_;
		size_t degree_;

		AssignmentReturn& operator = (const T& val) {
			if (!val) {
				if (degree_ >= coeffs_.size())
					return *this;
				if (degree_ + 1 == coeffs_.size()) {
					coeffs_.pop_back();
					while (coeffs_.size() && !coeffs_.back()) {
						coeffs_.pop_back();
					}
					return *this;
				}
		...
		operator T () const {
			return degree_ < coeffs_.size() ? coeffs_[degree_] : 0;
		}
	};
...
public:
	...
	AssignmentReturn operator [](size_t degree) { return { coeffs_, degree }; }
