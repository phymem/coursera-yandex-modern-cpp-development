...
public:
	...
	Access<V> operator[](const K& key) {
		Bucket& bucket = buckets[hasher(key) % buckets.size()];
		return { lock_guard<mutex>(bucket.mutex_), bucket.map_[key] };
	}
	Access<const V> At(const K& key) const {
		const Bucket& bucket = buckets[hasher(key) % buckets.size()];
		return { lock_guard<mutex>(bucket.mutex_), bucket.map_.at(key) };
	}

	bool Has(const K& key) const {
		const Bucket& bucket = buckets[hasher(key) % buckets.size()];
		lock_guard<mutex> lock(bucket.mutex_);
		return bucket.map_.count(key);
	}
	...
	MapType BuildOrdinaryMap() const {
		...
		for (const Bucket& bucket : buckets) {
			lock_guard<mutex> lock(bucket.mutex_);
			ret.insert(bucket.map_.begin(), bucket.map_.end());
		}
		return ret;
	}
