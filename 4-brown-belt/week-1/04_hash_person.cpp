...
struct AddressHasher {
	size_t operator() (const Address& addr) const {
		...
		const size_t prime = 780193;

		return hash_str(addr.city) * prime * prime
			+ hash_str(addr.street) * prime
			+ hash_int(addr.building);
	}
};
...
struct PersonHasher {
	size_t operator() (const Person& person) const {
		...
		hash<string> hash_str;
		AddressHasher hash_adr;
		hash<int> hash_int;

		const size_t prime = 123460289;

		return hash_str(person.name) * prime * prime * prime
			+ hash_int(person.height) * prime * prime
			+ hash_dbl(person.weight) * prime
			+ hash_adr(person.address);
	}
};
