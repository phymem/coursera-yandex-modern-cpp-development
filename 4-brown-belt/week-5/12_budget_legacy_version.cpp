...
void Add(size_t v, size_t l, size_t r, size_t ql, size_t qr, double income, double expense) {
	if (v >= VERTEX_COUNT || qr <= l || r <= ql) {
		return;
	}
	Push(v, l, r);
	...
	n_v->income_ =
		(v * 2 < VERTEX_COUNT ? tree[v * 2].income_: 0)
		+ (v * 2 + 1 < VERTEX_COUNT ? tree[v * 2 + 1].income_: 0);

	n_v->expense_ =
		(v * 2 < VERTEX_COUNT ? tree[v * 2].expense_: 0)
...
int main() {
	...
	for (int i = 0; i < q; ++i) {
		...
		if (query_type == "ComputeIncome") {
			...
			double value;
			cin >> value;
			Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, value / (idx_to - idx_from), 0 /*expense*/);
		} else if (query_type == "Spend") {
			double value;
			cin >> value;
			Add(1, 0, DAY_COUNT_P2, idx_from, idx_to, 0 /*income*/, value / (idx_to - idx_from));
		}
	}
