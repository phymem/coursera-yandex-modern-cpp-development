...
struct Record {
	...
	{ return Record{ lhs.income + rhs.income, lhs.expense + rhs.expense }; }
};
...
public:
	...
	BulkLinearUpdater(const BulkMoneySuber& sub) : sub_(sub) {}
	...
	Record Collapse(Record origin, IndexSegment segment) const {
		return Record {
			origin.income * tax_.factor + add_.delta * segment.length(),
			origin.expense + sub_.delta * segment.length() };
	}
...
struct ComputeIncomeRequest : ReadRequest<double> {
	...
	double Process(const BudgetManager& manager) const override {
		auto ret = manager.ComputeSum(MakeDateSegment(date_from, date_to));
		return ret.income - ret.expense;
	}
...
struct SpendRequest : ModifyRequest {
	...
	void Process(BudgetManager& manager) const override {
		...
		manager.AddBulkOperation(date_segment, BulkMoneySuber{daily_expense});
	}
