...
void TestSimple() {
	ASSERT_EQUAL(Split("moscow.m.ya.ru", "."),
		vector<string_view>({ "moscow", "m", "ya", "ru" }));

	{
		Domain domain("moscow.m.ya.ru");
		auto range = domain.GetReversedParts();
		vector<string> reversed(range.begin(), range.end());
		ASSERT_EQUAL(reversed, vector<string>({ "ru", "ya", "m", "moscow" }));
	}

	ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("m.ya.ru")));
	ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("ya.ru")));
	ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("ru")));
	ASSERT(!IsSubdomain(Domain("ya.ru"), Domain("m.ya.ru")));
	ASSERT(!IsSubdomain(Domain("ru"), Domain("m.ya.ru")));

	ASSERT_EQUAL(CheckDomains(
		vector<Domain>({ Domain("ya.com"), Domain("m.ya.ru"), Domain("ru") }),
		vector<Domain>({ Domain("ya.com"), Domain("m.maps.me"), Domain("m.ya.ru"), Domain("t.ru"), Domain("com") })),
		vector<bool>({ false, true, false, false, true}));

	{
		ostringstream os;
		PrintCheckResults(vector<bool>({ true, false, true, false }), os);
		ASSERT_EQUAL(os.str(), string("Good\nBad\nGood\nBad\n"));
	}

	{
		istringstream is("3\nya.ru\nmaps.me\ncom");
		ASSERT_EQUAL(ReadDomains(is), vector<Domain>({ Domain("ya.ru"), Domain("maps.me"), Domain("com") }));
	}

	{
		istringstream banned_is("4\nya.ru\nmaps.me\nm.ya.ru\ncom");
		const vector<Domain> banned_domains = ReadDomains(banned_is);
		istringstream check_is("7\nya.ru\nya.com\nm.maps.me\nmoscow.m.ya.ru\nmaps.com\nmaps.ru\nya.ya");
		const vector<Domain> domains_to_check = ReadDomains(check_is);

		ostringstream os;
		PrintCheckResults(CheckDomains(banned_domains, domains_to_check), os);

		ASSERT_EQUAL(os.str(), string("Bad\nBad\nBad\nBad\nBad\nGood\nGood\n"));
	}

	{
		vector<Domain> domains({ Domain("b.c"), Domain("a.b.c") });
		ASSERT(DomainChecker(domains.begin(), domains.end()).IsSubdomain(Domain("r.b.c")));
	}
}
